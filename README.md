...
- 通常の保存先・読込元は Cloudflare D1（`/api/harvest-records`）です。
- Google Sheets / Apps Script 連携は通常は未使用で、必要な場合だけ使うバックアップ用途です。
- GAS WebアプリURLの初期値は空です。設定画面で明示的に入力した場合のみ、D1保存後にバックアップ送信します。
- 共有キー認証は Cloudflare Pages Functions の `APP_SECRET` を使います。
- `APP_SECRET` と `OPENAI_API_KEY` の実値は、コードやREADMEへ記載しません。
...
- Google Sheets 連携は通常未使用のバックアップ用途として残しています
- OCR は `/api/read-weights` を使います
